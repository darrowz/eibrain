...
def test_sync_honjia_cli_runs_expected_ssh_and_scp(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["sync-honjia", "--target-host", "darrow@100.81.78.119", "--restart-monitor", "--include-tests"],
    )

    assert sync_honjia.main() == 0
    output = capsys.readouterr().out
    assert output.strip().startswith("synced=darrow@100.81.78.119:")
    assert calls[0][:2] == ["ssh", "darrow@100.81.78.119"]
    ...
    assert calls[-1][:4] == ["ssh", "darrow@100.81.78.119", "systemctl", "--user"]
